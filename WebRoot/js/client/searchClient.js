...
function init() {
	...
	$.ajax({
		type : "POST",
		url : "getClientQuery.do",
		dataType : "json",
		asycn : false,
		error : function(data) {
			alert("getClientQuery Error");
		},
		success : function(data) {
			$("#searchCompanyName").val(data.companyName);
			$("#searchPic").val(data.picName);
			$("#datepicker-autoclose").val(data.startDate);
			$("#datepicker-autoclose1").val(data.endDate);
			var trade = document.getElementById("searchStatus");
			for (var i = 0; i < trade.length; i++) {
				if (trade[i].value == data.state) {
					trade[i].selected = true;
					$("#searchStatus").selectpicker("refresh");
				}
			}
			var trade1 = document.getElementById("searchTrade");
			for (var i = 0; i < trade1.length; i++) {
				if (trade1[i].value == data.trade) {
					trade1[i].selected = true;
					$("#searchTrade").selectpicker("refresh");
				}
			}
		}
	})
				...
				success : function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						var leg = "<th style='text-align:left;vertical-align : middle; '>"
								...
								+ "</th>"
								+ "<th style='text-align:left;vertical-align : middle; '>"
								+ data[i].lastDate.substring(0,
										data[i].lastDate.indexOf("."))
								+ "</th></tr>";
